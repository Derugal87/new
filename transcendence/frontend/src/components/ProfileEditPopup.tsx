...
function ProfileEditPopup(props: ProfileEditPopupProps) {
	const storedUser = localStorage.getItem('user');
	let initialNickname = '';
	if (storedUser != undefined && !props.isHomePage) {
		initialNickname = JSON.parse(storedUser)?.nickname || '';
	}
	const [name, setName] = useState<string>(initialNickname);
		...
		if (isValid) {
			get('/user')
				.then((data: any) => {
					...
					console.log('initialNickname = ', initialNickname, name);
					if (isNameUnique.length === 0 || initialNickname === name) {
						setNameUnique(true);
	...
	const handleSaveFormClick = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		// let tmp_data: any = await put('/user/' + obj1.id_42, obj1).catch((e) => {
	...
	return (
		<form onSubmit={handleSaveFormClick} className="popup">
			<div className="popup-content">
				<h2 className="popup-content-title">{props.isHomePage ? '' : 'Edit '}Profile</h2>
				<div className="edit-image">
					<label htmlFor="image">{props.isHomePage ? '' : 'Change '}Image:</label>
					<input
						id="image"
						type="file"
						name="image"
				...
				<div className="edit-name">
					<label htmlFor="name">{props.isHomePage ? '' : 'Change '} Nickname: </label>
					<input
						autoFocus
				...
				<div className="buttons">
					...
					{!props.isHomePage && (
						<button type="button" onClick={props.onClosePopUpClick} className="cancel">
							Cancel
						</button>
					)}
				</div>
				{!props.isHomePage && (
					<button type="button" className="cancel-icon" onClick={props.onClosePopUpClick}>
